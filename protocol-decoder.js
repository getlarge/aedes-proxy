...
function isValidV1ProxyProtocol(buffer) {
  for (let i = 0; i < v1ProxyProtocolSignature.length; i += 1) {
...
function isValidV2ProxyProtocol(buffer) {
  for (let i = 0; i < v2ProxyProtocolSignature.length; i += 1) {
...
function protocolDecoder(client, data) {
  ...
  // var trustProxy = client.broker.trustProxy(client.conn.address);
